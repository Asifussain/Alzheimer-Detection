...
def predict():
    ...
    try:
        ...
        # # Generate Clinician PDF
        # clinician_pdf_url = None # Initialize
        # try:
        #     print("DEBUG: Attempting to generate Clinician PDF report...")
        #     clinician_pdf_doc = ClinicianPDFReport() 
        #     clinician_pdf_doc.alias_nb_pages() 
        #     _build_clinician_pdf_report_content( 
        #         clinician_pdf_doc,
        #         prediction_data_for_report,
        #         stats_json,
        #         similarity_analysis_results,
        #         consistency_metrics_results,
        #         ts_img_data,
        #         psd_img_data,
        #         similarity_plot_base64_data 
        #     )
        #     clinician_pdf_bytes = bytes(clinician_pdf_doc.output())
        #     clinician_pdf_filename = f"{asset_prefix}/clinician_summary_report.pdf" 
        #     print(f"DEBUG: Clinician PDF filename for Supabase: {clinician_pdf_filename}")

        #     upload_response = supabase.storage.from_(REPORT_ASSET_BUCKET).upload(
        #         path=clinician_pdf_filename,
        #         file=clinician_pdf_bytes,
        #         file_options={"content-type": "application/pdf", "upsert": "true"}
        #     )
        #     print(f"DEBUG: Clinician PDF Supabase upload response: {upload_response}")


        #     clinician_pdf_url_res = supabase.storage.from_(REPORT_ASSET_BUCKET).get_public_url(clinician_pdf_filename)
        #     print(f"DEBUG: Raw clinician_pdf_url_res from Supabase GET_PUBLIC_URL: {clinician_pdf_url_res}")

        #     # Check the type and content of clinician_pdf_url_res
        #     if isinstance(clinician_pdf_url_res, str) and clinician_pdf_url_res.startswith('http'):
        #         clinician_pdf_url = clinician_pdf_url_res
        #     else:
        #         # If get_public_url does not return a string or a valid URL, clinician_pdf_url remains None
        #         clinician_pdf_url = None 
        #         print(f"Warning: get_public_url for clinician PDF did not return a valid URL string. Received: {clinician_pdf_url_res}")

        #     print(f"DEBUG: Processed clinician_pdf_url (this will be saved to DB): {clinician_pdf_url}")
            
        #     if not clinician_pdf_url:
        #         report_generation_errors.append("ClinicianPDFURL")
        #         print("Warning: Clinician PDF URL is None after processing Supabase response.")
        # except Exception as e_clin_pdf:
        #     print(f"CRITICAL ERR ClinicianPDF Generation/Upload: {e_clin_pdf}")
        #     traceback.print_exc()
        #     report_generation_errors.append("ClinicianPDFGen")
        #     clinician_pdf_url = None # Ensure it's None on error
