...
class ClinicianPDFReport(BasePDFReport):
    def __init__(self, *args, **kwargs):
        ...
        # Clinician-specific color overrides or use BasePDFReport's
        self.primary_color = (41, 128, 185) # Slightly different blue for clinician
...
def build_clinician_pdf_report_content(pdf: ClinicianPDFReport, prediction_data, stats_data, 
    ...
    try:
        ...
        # --- Section I: Patient & Analysis Summary ---
        ...
        primary_finding = "Pattern Suggestive of Alzheimer's-related Changes" if prediction_label == "Alzheimer's" else "Normal EEG Pattern" if prediction_label == "Normal" else "Indeterminate"
        ...
        if isinstance(probabilities, list) and len(probabilities) == 2:
            ...
            except Exception: prob_str = sanitize_for_helvetica(str(probabilities))
        elif probabilities is not None: prob_str = sanitize_for_helvetica(str(probabilities))
        pdf.key_value_pair("Assessment Confidence", prob_str)
        pdf.ln(5)

        # --- Section II: EEG Pattern Analysis & Consistency ---
        ...
        if consistency_metrics and not consistency_metrics.get('error'):
            ...
            if num_trials > 1: # Only provide detailed consistency if multiple segments were analyzed
                accuracy = consistency_metrics.get('accuracy', 0)
                if accuracy >= 0.85: consistency_statement = f"High (Overall segment agreement: {format_metric_for_pdf(accuracy, 'percent',0)} across {num_trials} segments)"
                elif accuracy >= 0.70: consistency_statement = f"Moderate (Overall segment agreement: {format_metric_for_pdf(accuracy, 'percent',0)} across {num_trials} segments)"
                else: consistency_statement = f"Low (Overall segment agreement: {format_metric_for_pdf(accuracy, 'percent',0)} across {num_trials} segments. Interpret with caution.)"
        ...
        # --- Section III: Key EEG Waveform Characteristics (from Similarity Analysis) ---
        ...
        if similarity_data and not similarity_data.get('error'):
            ...
            pdf.set_font("Helvetica",'I',10); pdf.write_multiline(err_msg_sim_clin, indent=5)
        pdf.ln(10)


        if pdf.get_y() > pdf.h - 100 : pdf.add_page()
        # --- Section IV: Brainwave Frequency Profile ---
        ...
        if stats_data and not stats_data.get('error') and stats_data.get('avg_band_power'):
            ...
            if band_data_points:
                # Simple list format for clinician
                for point in band_data_points:
                    pdf.cell(10); pdf.cell(0,5,point, ln=1)
            else:
            ...
            pdf.set_font("Helvetica",'I',10); pdf.write_multiline(err_msg_stats_clin, indent=5)
        pdf.ln(10)
        

        if pdf.get_y() > pdf.h - 120 : pdf.add_page()
        # --- Section V: EEG Visualizations (Optional, include if relevant for clinician) ---
        ...
        # --- Section VI: Important Notes & Disclaimer ---
        if pdf.get_y() > pdf.h - 50 : pdf.add_page()
        ...
        pdf.set_text_color(80,80,80) # Slightly muted color for disclaimer
