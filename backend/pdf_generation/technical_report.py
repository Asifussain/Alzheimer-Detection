...
def format_metric_for_pdf(value, type='float', precision=1):
    """ Helper to format metrics consistently for PDF, handling N/A for errors. """
    if value is None or (isinstance(value, float) and (pd.isna(value) or not pd.Series(value).notna().all())): # handles NaN
        return 'N/A'
    try:
        if type == 'percent':
    ...
    try:
        ...
        if consistency_metrics and not consistency_metrics.get('error') and consistency_metrics.get('num_trials', 0) > 0:
            ...
            def add_metric_row(metric1_args, metric2_args=None):
                ...
                if metric2_args:
                    pdf.set_y(current_y) # Reset Y for the second card in the row
                    pdf.metric_card(*metric2_args)
                pdf.ln(28) # Approximate height of metric_card + spacing
            ...
            pdf.set_font('Helvetica', 'I', 10); pdf.write_multiline("(Internal consistency metrics not calculated or not applicable.)", indent=5)
        pdf.ln(5)
        ...
        if similarity_data and not similarity_data.get('error'):
            ...
            if similarity_plot_data:
                ...
                pdf.add_image_section(plot_title_sim, similarity_plot_data) # uses BasePDFReport
            else:
                pdf.set_font("Helvetica",'I',10); pdf.cell(0,10,"(Similarity plot not generated or invalid)", ln=1)
        else:
            err_msg_sim = similarity_data.get('error', 'Unknown') if similarity_data else 'data not available'
            pdf.set_font("Helvetica",'I',10); pdf.write_multiline(f"(Similarity Analysis Error: {err_msg_sim})", indent=5)
        pdf.ln(5)
        ...
        if stats_data and not stats_data.get('error'):
            ...
            if not band_found:
                pdf.set_font("Helvetica",'I',10); pdf.cell(10); pdf.cell(0,5,"(No band power data)", ln=1)
            pdf.ln(3)

            std_devs = stats_data.get('std_dev_per_channel')
            if std_devs:
                pdf.set_font("Helvetica",'B',11); pdf.cell(0,6,"Standard Deviation per Channel (µV):", ln=1); pdf.ln(1)
                std_dev_str = ", ".join([format_metric_for_pdf(s, 'float', 2) for s in std_devs])
                pdf.set_font("Helvetica",'',9); pdf.write_multiline(std_dev_str, indent=2)
        else:
            err_msg_stats = stats_data.get('error', 'Unknown') if stats_data else 'data not available'
            pdf.set_font("Helvetica",'I',10); pdf.write_multiline(f"(Statistics Error: {err_msg_stats})", indent=5)
        pdf.ln(5)
